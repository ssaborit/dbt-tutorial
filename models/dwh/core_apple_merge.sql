...
WITH
  ...
  WHERE
    ...
    AND DATE(t_store.date) >= date_sub(CURRENT_DATE(), INTERVAL {{ var('offset_days') }} DAY)
      -- and DATE(t_store.date) between '{{ var('start_date') }}' and '{{var('end_date')}}'
    {% endif %}
  ),
  ...
  WHERE
    ...
    AND DATE(t_store_app.date) >= date_sub(CURRENT_DATE(), INTERVAL {{ var('offset_days') }} DAY)
      -- and DATE(t_store_app.date) between '{{ var('start_date') }}' and '{{var('end_date')}}'
    {% endif %}
  ...
  WHERE
    ...
    AND DATE(t_store_web.date) >= date_sub(CURRENT_DATE(), INTERVAL {{ var('offset_days') }} DAY)
      -- and DATE(t_store_web.date) between '{{ var('start_date') }}' and '{{var('end_date')}}'
    {% endif %}
...
SELECT
    ...
    totals.territory,
    totals.source_type,
    coalesce(app_referrer, '{{ var('na_value') }}') AS app_referrer,
    coalesce(web_referrer, '{{ var('na_value') }}') AS web_referrer,
    CASE WHEN (app_referrer <> '{{ var('na_value') }}' or web_referrer <> '{{ var('na_value') }}') then referrer_with_others.store_impressions ELSE totals.store_impressions END              AS store_impressions,
    CASE WHEN (app_referrer <> '{{ var('na_value') }}' or web_referrer <> '{{ var('na_value') }}') then referrer_with_others.store_page_views ELSE totals.store_page_views END                AS store_page_views,
    CASE WHEN (app_referrer <> '{{ var('na_value') }}' or web_referrer <> '{{ var('na_value') }}') then referrer_with_others.store_impressions_raw ELSE totals.store_impressions_raw END      AS store_impressions_raw,
    CASE WHEN (app_referrer <> '{{ var('na_value') }}' or web_referrer <> '{{ var('na_value') }}') then referrer_with_others.store_page_views_raw ELSE totals.store_page_views_raw END        AS store_page_views_raw,
    CASE WHEN (app_referrer <> '{{ var('na_value') }}' or web_referrer <> '{{ var('na_value') }}') then referrer_with_others.store_downloads ELSE totals.store_downloads END                  AS store_downloads,
    CASE WHEN (app_referrer <> '{{ var('na_value') }}' or web_referrer <> '{{ var('na_value') }}') then referrer_with_others.store_in_app_purchases ELSE totals.store_in_app_purchases END    AS store_in_app_purchases,
    CASE WHEN (app_referrer <> '{{ var('na_value') }}' or web_referrer <> '{{ var('na_value') }}') then referrer_with_others.store_installs ELSE totals.store_installs END                    AS store_installs,
    CASE WHEN (app_referrer <> '{{ var('na_value') }}' or web_referrer <> '{{ var('na_value') }}') then referrer_with_others.store_sessions ELSE totals.store_sessions END                    AS store_sessions,
    CASE WHEN (app_referrer <> '{{ var('na_value') }}' or web_referrer <> '{{ var('na_value') }}') then referrer_with_others.store_active_devices ELSE totals.store_active_devices END        AS store_active_devices
FROM totals
...
SELECT f.*, {{dbt_utils.surrogate_key(['date', 'product_id', 'app_id', 'territory', 'source_type', 'app_referrer', 'web_referrer'])}} as pk_core_apple
FROM final_table f
